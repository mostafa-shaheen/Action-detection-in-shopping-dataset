...
class CroppedVideoDataset(Dataset):
    """Dataset Class for Loading Video"""

    def __init__(self,videos_dir, label_dir, training_type, chunk_step,channels,width,hight, transform=None):
        ...
        if training_type == 'training':
            self.video_file_list = [video for video in self.video_file_list if int(video[8:-11])<21]
            self.label_file_list = [label for label in self.label_file_list if int(label[:-12])<21]
        elif training_type == 'testing':
            self.video_file_list = [video for video in self.video_file_list if int(video[8:-11])>26]
            self.label_file_list = [label for label in self.label_file_list if int(label[:-12])>26]
        elif training_type == 'validation':
            self.video_file_list = [video for video in self.video_file_list if 20<(int(video[8:-11]))<27]
            self.label_file_list = [label for label in self.label_file_list if 20<(int(label[:-12]))<27]


        print(self.video_file_list)
        print(self.label_file_list)
